...
class GenerateExternalDocsHandlerTest {
  ...
  void shouldLogAPIsWhenInvoked() {
    setupSimpleMocks();
    var logRecorder = LogRecorder.forRoot(GenerateExternalDocsHandlerTest.class);
  ...
  void shouldNotIncludedNonExternalPaths() {
    var fileNames = List.of("api-a.yaml");
    setupTestCasesFromFiles(null, fileNames);
  ...
  void shouldIncludeExternalSchemaWhenItsDirectlyReferencedFromResponse() {
    ...
    assertThat(
        openapi.getComponents().getSchemas().containsKey("NestedExternalSchema"), equalTo(true));
  }
  ...
  void shouldIncludeExternalSchemaWhenItsReferencedFromRequest() {
    ...
    assertThat(
        openapi.getComponents().getSchemas().containsKey("ExternalRequestSchema"), equalTo(true));
  }
