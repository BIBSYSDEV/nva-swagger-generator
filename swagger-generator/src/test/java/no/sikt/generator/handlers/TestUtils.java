  ...
  public static void loadGithubOpenapiFile(S3Driver s3Driver, TestCase testCase) {
    if (testCase.getContentGithub().isPresent()) {
      Try.attempt(
          () ->
              s3Driver.insertFile(
                  UnixPath.of(String.valueOf(Path.of(testCase.getId() + ".yaml"))),
                  testCase.getContentGithub().get()));
    }
  }

  public static void setupTestcasesFromFiles(
      S3Client s3Client,
    ...
    var getDocPartsResponse =
        GetDocumentationPartsResponse.builder()
            .items(
                DocumentationPart.builder()
                    .id("1")
                    .location(docPartLoc1)
                    .properties("prop1")
                    .build(),
                DocumentationPart.builder()
                    .id("2")
    ...
    when(apiGatewayAsyncClient.createDocumentationVersion(
            any(CreateDocumentationVersionRequest.class)))
        .thenReturn(CompletableFuture.completedFuture(createDocumentationVersionResponse));
    when(apiGatewayAsyncClient.deleteDocumentationVersion(
            any(DeleteDocumentationVersionRequest.class)))
